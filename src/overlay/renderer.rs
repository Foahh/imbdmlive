...
impl D3D9RenderEngine {
    pub fn new(device: &IDirect3DDevice9, ctx: &mut Context) -> Result<Self> {
        ...
        ctx.set_renderer_name(String::from(concat!("imbdmlive-dx9@", env!("CARGO_PKG_VERSION"))));
    ...
    /// Build the font atlas texture and register it with the context.
    pub fn setup_fonts(&mut self, ctx: &mut Context) -> Result<()> {
        let fonts = ctx.fonts();
    ...
    pub fn render(&mut self, draw_data: &DrawData, render_target: IDirect3DSurface9) -> Result<()> {
        unsafe {
...
impl StateBackup {
    unsafe fn backup(device: &IDirect3DDevice9) -> Result<Self> {
        ...
        Ok(StateBackup { state_block, mat_world, mat_view, mat_projection, viewport, surface })
    }
